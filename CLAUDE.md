...
reitrn.com (the Lite product). No cloud connectivity â€” just an HTTP API on
localhost. Repo: github.com/reitrn/reitrn-print-agent-lite. Windows-only.

## Git: use `main`

History is unusual â€” record for future sessions:
...
- On 2026-06-10 the full working tree was committed on `master` and merged into
  `main`. **`main` is now canonical â€” always branch from and push to `main`.**

## Stack

- Electron 28 + electron-builder (NSIS + portable, x64)
- `electron-store` only â€” no Firebase, no remote registration, fast startup
...
- HTTP (not HTTPS) server on `127.0.0.1:3010` â€” same port as the full agent, so
  **don't run both on one PC**. Endpoints: `/ping`, `/status`, `POST /print`
  (body: `{ data }` or `{ zpl }` / `{ tspl }` â€” used as-is, no rendering).

## Commands

- `npm start` â€” run locally
- `npm run build` â€” installer + portable (`reitrn-print-agent-lite-setup.exe`)

## Key files

- `main.js` â€” tray/window, localhost server, IPC (`getState`, `testPrint`, â€¦)
- `printer.js` â€” printer listing + raw print fallback chain
- `renderer/` â€” plain HTML/JS settings UI

## Decisions & gotchas

- **Windows 11 printer listing**: `wmic` is removed on Win 11 22H2+. `printer.js`
  tries native module â†’ `wmic` â†’ **PowerShell `Get-Printer` fallback** (the fix
...
- Raw print fallback chain: direct Node `fs` write to `\\.\USBxxx` (port via wmic,
  cached per printer) â†’ PowerShell CreateFile P/Invoke â†’ `copy /b` to
  `\\localhost\<printer>` â†’ WinSpool P/Invoke.
...
- No secrets in this repo and none should be added â€” it is intentionally
  cloud-free.
